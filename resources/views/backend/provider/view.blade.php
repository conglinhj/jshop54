...
@section('content')
    ...
    <div class="page-heading">
        <h1><i class='icon-info-circled-2'></i> Thông tin chi tiết</h1>
    ...
    <div class="row">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="toolbar-btn-action">
                                    <a href="{{ route('backend.provider.showEditForm',['id' => $details_provider['id']]) }}" class="btn btn-success"><i class="fa fa-edit"></i> Edit</a>
                                    <a class="btn btn-danger"
                                       onclick="event.preventDefault();document.getElementById('provider_form_delete').submit();" >
                                        <i class="fa fa-trash-o"></i> Delete</a>
                                    <form id="provider_form_delete" action="{{ route('backend.provider.destroy') }}" method="post" style="display: none;">
                                        {{ csrf_field() }}
                                        <input type="hidden" value="{{ $details_provider['id'] }}" name="provider_id">
                                    </form>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="table-responsive">
                        <table data-sortable class="table table-hover table-striped">
                            ...
                            <tr>
                                ...
                                <td><strong>{{ $details_provider['name'] }}</strong></td>
                            </tr>
                            <tr>
                                <td>Email</td>
                                <td>{{ $details_provider['email'] }}</td>
                            </tr>
                            <tr>
                                <td>Số điện thoại</td>
                                <td>{{ $details_provider['phone'] }}</td>
                            </tr>
                            <tr>
                                <td>Địa chỉ</td>
                                <td>{{ $details_provider->address.', '.$details_provider->township->name.', '.$details_provider->county->name.', '.$details_provider->city->name }}</td>
                            </tr>
                            <tr>
                                <td>Ngày tạo</td>
                                <td>{{ $details_provider['created_at'] }}</td>
                            </tr>
        ...
        <div class="col-md-12">
            <div class="widget">
                <div class="widget-header transparent">
                    <h2><strong>Đơn nhập hàng từ {{ $details_provider['name'] }}</strong></h2>
                    <div class="additional-btn">
                        <a href="#" class="hidden reload"><i class="icon-ccw-1"></i></a>
                        <a href="#" class="widget-toggle"><i class="icon-down-open-2"></i></a>
                    </div>
                    ...
                    <div id="basic-form">
                        <form id="specs_form" method="post" role="form" style="display: none;">
                            ...
                            <div id="specs-block" class="form-group col-md-6">
                                ...
                                <input type="hidden" name="provider_id" value="{{ $details_provider['id'] }}">
